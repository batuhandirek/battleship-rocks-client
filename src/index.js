    ...
    const serverResults = await Promise.all(
      servers.map(async (server) => {
        ...
        try {
          ...
          console.info(
            `[ServerSelection] Server ${server.alias} pong: ${result.time}ms`,
          );
          return result;
        } catch (e) {
  ...
  try {
    ...
    if (ok && tokenSaved) {
      ...
      if (continueRes.ok) {
        ...
        continueRes.error &&
        continueRes.error.code === "multiple-window"
      ) {
        console.error(
          "Cannot open the game from multiple windows! Killing this one gracefully in 2 seconds.",
        );
        await delay(2000);
        process.exit(0);
      ...
      if (!createdRes.ok)
        throw createdRes.error ?? new Error("Could not create a session.");
