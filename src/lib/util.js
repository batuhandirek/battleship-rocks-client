...
export const generateEmptyBoard = (size) => {
    return new Array(size).fill(1).map(one => 
        new Array(size).fill(1).map(one => 0)
    )
}
...
export const paintBoard = (board, { row, col, size, direction }, value) => {
    if(direction === 'horz') {
        new Array(size).fill(1).forEach((one, index) => {
            if([row, col+index].some(i => i < 0 || i >= board.length)) return board
            board[row][col + index] = value
        })
    } else {
        new Array(size).fill(1).forEach((one, index) => {
            if([col, row + index].some(i => i < 0 || i >= board.length)) return board
...
export const placeShip = (board, size) => {
    ...
    if(direction === 'horz') {
        ...
        for (let col = 0; col < boardState.length; col++) {
            for (let row = 0; row <= boardState.length - size; row++) {
                const slice = new Array(size).fill(1).map((o, index) => boardState[row+index][col])
                const isValid = slice.every(el => el === 0)
    ...
    newBoard = paintBoard(newBoard, { size: vertSizeDelete, direction: 'vert', row: placed.row -1, col: placed.col -1 }, 5)
    newBoard = paintBoard(newBoard, { size: vertSizeDelete, direction: 'vert', row: placed.row -1, col: placed.col + horzSizeDelete }, 5)
    newBoard = paintBoard(newBoard, { size: horzSizeDelete, direction: 'horz', row: placed.row - 1, col: placed.col }, 5)
    newBoard = paintBoard(newBoard, { size: horzSizeDelete, direction: 'horz', row: placed.row + vertSizeDelete - 2, col: placed.col }, 5)
...
export const randomizeBoard = (rowCount, iter = 0) => {
    ...
    try {
        ...
        if(e.message === 'no-possible-place') return randomizeBoard(rowCount, iter + 1)
    }
}
